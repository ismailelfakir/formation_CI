...
TESettingsFromTracks.prototype = {
	ready: function (ctrl) {
		...
		if (transcriptTrack) {
			...
			if (transcriptTrack.localName == 'html-track') {
				...
				transcriptLink.title = "Display transcript";
				transcriptLink.href = transcriptSrc;
				...
				if (transcriptTarget == "subWindow") {
					...
					else transcriptWinOptions = {CLOSEBTNTI :'Close'};
	...
	createSubtitleItem: function (trackElem) {
		...
		var title = langOrLabel ?  "Activate subtitles " + langOrLabel : "Deactivate the subtitles";
		var label = langOrLabel || "Deactivated";
